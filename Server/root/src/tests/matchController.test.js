...
describe("Match Controller", () => {
  ...
  beforeEach(() => {
    ...
    token = jwt.sign({ id: "testUserId" }, process.env.SUPER_SECRET, {
      expiresIn: "1h",
    });
  });
  ...
  describe("PUT /api/matches/status/:matchID", () => {
    it("should update match status successfully", async () => {
      ...
      const expectedMatch = {
        ...mockMatch,
        matchStatus: matchStatusEnum.ACCEPTED,
      };
      delete expectedMatch.save;

      expect(response.status).toBe(200);
      expect(response.body).toEqual({
        message: `Match status updated to ${matchStatusEnum.ACCEPTED}`,
        match: expectedMatch,
      });
    });
  ...
  describe("POST /api/matches/:matchID/messages", () => {
    it("should add a message to match successfully", async () => {
      ...
      // Mock the push method to behave correctly
      const pushSpy = jest
        .spyOn(mockMatch.messages, "push")
        .mockImplementation((msg) => {
          mockMatch.messages = [...mockMatch.messages, msg];
        });
      ...
      const expectedMatch = {
        ...mockMatch,
        messages: [{ text: "Hello!", userID: "testUserId" }],
      };
      delete expectedMatch.save;
  ...
  describe("DELETE /api/matches/:matchID", () => {
    ...
    it("should handle errors and return 500 status code", async () => {
      MatchModel.findByIdAndDelete.mockRejectedValue(
  ...
  describe("PUT /api/matches/:matchID", () => {
    ...
    it("should handle errors and return 500 status code", async () => {
      MatchModel.findByIdAndUpdate.mockRejectedValue(
        new Error("Database error")
      );
