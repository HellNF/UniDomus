...
//const { formatDate, calculateAge} = require('../utils/dateUtils');
...
export default function TenantDetails() {
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()}>
      <div className="bg-blue-950 min-h-screen flex flex-col items-center p-8">
        ...
        <div className="bg-white w-full max-w-4xl mt-6 rounded-lg p-8 shadow-md">
          <div className="flex justify-between space-x-4">
            ...
            <div className="flex-1 flex flex-col py-8 space-y-2">
              ...
              {isLoggedIn && user._id !== userId ? (<button className="bg-red-500 font-bold text-white p-2 rounded-md m-2">Segnala</button>) : (<></>)}
                {isLoggedIn && user._id === userId ? (<Link className="bg-blue-950 font-bold text-white p-2 rounded-md m-2" >Modifica</Link>) : (<></>)}
                {isLoggedIn && user._id !== userId ? (<button className="bg-blue-950 font-bold text-white p-2 rounded-md m-2" >
                  {
                    !isLiked ?
